...
public class AgencyTypeGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all agencyTypes").get("/api/agency-types").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new agencyType")
                        .post("/api/agency-types")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" + "\"name\": \"SAMPLE_TEXT\"" + ", \"shortName\": \"SAMPLE_TEXT\"" + ", \"isActive\": null" + "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_agencyType_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created agencyType").get("${new_agencyType_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created agencyType").delete("${new_agencyType_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the AgencyType entity").exec(scn);
