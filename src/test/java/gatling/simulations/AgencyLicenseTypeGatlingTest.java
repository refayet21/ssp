...
public class AgencyLicenseTypeGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all agencyLicenseTypes")
                    .get("/api/agency-license-types")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new agencyLicenseType")
                        .post("/api/agency-license-types")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"name\": \"SAMPLE_TEXT\"" + ", \"isActive\": null" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_agencyLicenseType_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created agencyLicenseType").get("${new_agencyLicenseType_url}").headers(headers_http_authenticated)
                    ).pause(10)
                )
                .exec(http("Delete created agencyLicenseType").delete("${new_agencyLicenseType_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the AgencyLicenseType entity").exec(scn);
