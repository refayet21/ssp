...
public interface PersonRepository extends JpaRepository<Person, Long> {
    ...
    @Query(
        value = "select person from Person person left join fetch person.internalUser left join fetch person.nationality",
        countQuery = "select count(person) from Person person"
    )
    Page<Person> findAllWithToOneRelationships(Pageable pageable);

    @Query("select person from Person person left join fetch person.internalUser left join fetch person.nationality")
    List<Person> findAllWithToOneRelationships();

    @Query("select person from Person person left join fetch person.internalUser left join fetch person.nationality where person.id =:id")
