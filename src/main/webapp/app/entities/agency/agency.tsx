  ...
  return (
    <div>
      <h2 id="agency-heading" data-cy="AgencyHeading">
        <Translate contentKey="cpamainApp.agency.home.title">Agencies</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="cpamainApp.agency.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/agency/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="cpamainApp.agency.home.createLabel">Create new Agency</Translate>
          </Link>
        </div>
      </h2>
        ...
        {agencyList && agencyList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="cpamainApp.agency.id">ID</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                <th className="hand" onClick={sort('name')}>
                  <Translate contentKey="cpamainApp.agency.name">Name</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('name')} />
                </th>
                ...
                <th className="hand" onClick={sort('isInternal')}>
                  ...
                  <FontAwesomeIcon icon={getSortIconByFieldName('isInternal')} />
                </th>
                <th className="hand" onClick={sort('isDummy')}>
                  <Translate contentKey="cpamainApp.agency.isDummy">Is Dummy</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('isDummy')} />
                </th>
                <th>
                  <Translate contentKey="cpamainApp.agency.agencyType">Agency Type</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {agencyList.map((agency, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{agency.isInternal ? 'true' : 'false'}</td>
                  <td>{agency.isDummy ? 'true' : 'false'}</td>
                  <td>{agency.agencyType ? <Link to={`/agency-type/${agency.agencyType.id}`}>{agency.agencyType.name}</Link> : ''}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="cpamainApp.agency.home.notFound">No Agencies found</Translate>
            </div>
          )
        )}
      </div>
