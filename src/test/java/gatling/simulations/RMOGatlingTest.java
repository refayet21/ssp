...
public class RMOGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all rMOS").get("/api/rmos").headers(headers_http_authenticated).check(status().is(200)))
                ...
                .exec(
                    http("Create new rMO")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_rMO_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created rMO").get("${new_rMO_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created rMO").delete("${new_rMO_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the RMO entity").exec(scn);
