...
public class PassGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all passes").get("/api/passes").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new pass")
                        .post("/api/passes")
                        ...
                        .body(
                            StringBody(
                                ...
                                ", \"status\": \"REQUESTED\"" +
                                ", \"passNumber\": 0" +
                                ", \"mediaSerial\": \"SAMPLE_TEXT\"" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_pass_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created pass").get("${new_pass_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created pass").delete("${new_pass_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Pass entity").exec(scn);
