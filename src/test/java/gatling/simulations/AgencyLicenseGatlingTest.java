...
public class AgencyLicenseGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all agencyLicenses").get("/api/agency-licenses").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new agencyLicense")
                        .post("/api/agency-licenses")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"filePath\": \"SAMPLE_TEXT\"" +
                                ", \"serialNo\": \"SAMPLE_TEXT\"" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_agencyLicense_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created agencyLicense").get("${new_agencyLicense_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created agencyLicense").delete("${new_agencyLicense_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the AgencyLicense entity").exec(scn);
