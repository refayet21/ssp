...
public class VehicleAssignmentGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all vehicleAssignments")
                ...
                .exec(
                    http("Create new vehicleAssignment")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_vehicleAssignment_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created vehicleAssignment").get("${new_vehicleAssignment_url}").headers(headers_http_authenticated)
                    ).pause(10)
                )
                .exec(http("Delete created vehicleAssignment").delete("${new_vehicleAssignment_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the VehicleAssignment entity").exec(scn);
